...
def load_nifty200_bhavcopy():
    ...
    if not files:
        raise FileNotFoundError("No Nifty 200 bhavcopy found")
    latest = max(files)
...
def main():
    print("\n" + "="*60)
    print("NSE MOMENTUM SCANNER - NIFTY 200")
    print("="*60)
    ...
    try:
        ...
        buy_watchlist = df[df['RECOMMENDATION'].isin(['BUY', 'STRONG_BUY'])].head(30)
        ...
        print(f"   Top 30 watchlist saved: {len(buy_watchlist)} stocks")
